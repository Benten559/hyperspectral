%Batch
wd = 'V:\Data\Lab_Data\Leaf_Cabinet_Imagery\Leaf_2020_Cabinet';
localWD = 'C:\Users\15593\Desktop\Interlaced Radiance Average\Test Images';
files = dir(wd);    % Assumes '.dat' folders within working directorsk
leafId = {files([files.isdir]).name};
leafId = string(leafId(~ismember(leafId,{'.','..'})));
leafId = leafId(strlength(leafId)>31);
% additionally, remove some files with invalid ID's
...
%        reflCube = zeros(603,681,80);%data cube to consist of reflectance values WHERE EVERYTHING WILL BE WRITTEN
        id = idNames(:,19:37,i)
        %TESTING: Different image error folders
        %if ~isfile(fullfile(localWD,'Redo','OutlineOnly',id+string('.jpg') ))%(localWD,'Phase1',id+string('.jpg')))
        %if ~isfile(fullfile(localWD,'Redo','Off',id+string('.jpg') ))%(localWD,'Phase1',id+string('.jpg')))
%        if ~isfile(fullfile(localWD,'Redo','OffRound1Fail',id+string('.jpg') ))    
%            continue;
%        end
        %h5create("..\leafh5\" +id + ".h5", '/img',size(reflCube)); % create h5 in parent dir
        datpath1000ms = fullfile(wd,leafId(i),exposures(1),imgsFolder(1),dat(1));
        datpath200ms = fullfile(wd,leafId(i),exposures(2),imgsFolder(2),dat(2));
        % acquire data cubes
        ...
        rgb = colorize(Cube,'Method','CIR','ContrastStretching',true);%colorize(Cube);
...
%        else
%            if isfile(fullfile( localWD,'Redo','Off',id+string('.jpg') )) %handle some bad standard identification
%                segmentation = segmentation ~= leafCluster;
%            else
...
%        end
        ...
        % Overlay circle mask
        [fullCircle,outline] = segCircle(rgb(:,:,1));%(cast(segmentation,'double')); % logical circle outline
        
        % when mask = fullcircle, the leaf is actually the standard
        mask = fullCircle ~= outline ;%outline ~= 0; %WHEN SEG ~=LEAFCLUSTER, mask is inner circle
%        leaf = segmentation.*mask;
        %store image
        img = Cube.DataCube(:,:,40);
%        if isfile(fullfile(localWD,'Redo','OutlineOnly',id+string('.jpg'))) % hande outlined standard images
        saveas(imagesc([img img.*mask]),fullfile('C:\Users\15593\Desktop\Interlaced Radiance Average\Test Images',id + string('.jpg'))) ;
        
%            saveas(imagesc([img img.*mask]),fullfile('C:\Users\15593\Desktop\Interlaced Radiance Average\Test Images',id + string('.jpg'))) ;
%        elseif isfile(fullfile(localWD,'Redo','Off',id+string('.jpg')))
%            saveas(imagesc([img img.*mask]),fullfile('C:\Users\15593\Desktop\Interlaced Radiance Average\Test Images',id + string('.jpg'))) ;
%        else 
%            saveas(imagesc([img img.*mask]),fullfile('C:\Users\15593\Desktop\Interlaced Radiance Average\Test Images',id + string('.jpg'))) ;
%        end
%        imgSaveInput = imagesc([img img.*mask])
%        else
%            imgSaveInput = imagesc([img img.*outline])
%        end
        
        %saveas(imgSaveInput,fullfile('C:\Users\15593\Desktop\Interlaced Radiance Average\Test Images',id + string('.jpg'))) ;
        %imwrite(img,fullfile('C:\Users\15593\Desktop\Interlaced Radiance Average\Test Images',id + string('.jpg'))) ;
        
    catch ME
        error = "skipping: " + id
        %warning('skipping :'+ id+ ' '+ ME.identifier);
        continue;
    end
    %to do: re-insert 500 & 505 wv's to make length == 80
    %% Uncomment block
    for j = 1:length(wv)
        % TO WRITE REFLECT CUBE
        ...
        % FOR AVERAGING:
        img = Cube.DataCube(:,:,j).*mask.*dataGain(j);
        %if isfile(fullfile(localWD,'Redo','Off',id,'.jpg'))
        %    img = Cube.DataCube(:,:,j).*mask.*dataGain(j);
        %elseif isfile(fullfile(localWD,'Redo','OffRound1Fail',id+string('.jpg') ))  
        %    img = Cube.DataCube(:,:,j).*mask.*dataGain(j);
        %end
...
%   h5write("..\leafh5\" +id + ".h5", '/img',reflCube);
    ...
    % LOG AVERAGES
    writematrix([id string(avgxband)],'C:\Users\15593\Desktop\Interlaced Radiance Average\Test Radiance Sheets\InterlacedRadAvg.csv','WriteMode','append');
