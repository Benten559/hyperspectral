...
leafMode <- "cropped"
standard200 <- "leaf_220ms_80wl_000000"
standard1000 <- "leaf_1000ms_80wl_000000"
...
grabCroppedStandard <- function(iTH_id_dir)
{
  standardExposures <- list("leaf_2020_1000ms_80_000000","leaf_2020_200ms_80wl_000000")
  twoExposures <- list.files(iTH_id_dir)[1:2]
  croppedVector <- list()
  for(i in 1:2)
  {
    ins <-paste(twoExposures[i],standardExposures[i],sep = '/')
    croppedVector <- c(croppedVector,ins)
  }
...
for( i in 1:length(leafIdList))
{
  ...
  exposures <- grabCroppedStandard(iTH_image_dir)
  #str(iTH_image_dir)
  ...
  write.table(df, file = "C:/Users/Ben's PC/Documents/research/hyperspectral/1000msStandardRadianceAvg.csv", row.names = FALSE,sep = ',',col.names = FALSE ,append = TRUE)
  str("finished Write\n")
