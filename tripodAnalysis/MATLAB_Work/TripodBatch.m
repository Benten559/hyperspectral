...
%% PATH VARIABLES
files = dir('D:\St_Supery_IOP1_Sep2020_contd');    % SSD#5
%files = dir('D:\St_Supery_IOP1_Sep2020');           %SSD #1
%files = dir('D:\Tripod_data\Sep_2020');           %SSD #4
leafId = {files([files.isdir]).name}; 
leafId = string(leafId(~ismember(leafId,{'.','..'}))); %Thsle working image folder
exposures = string(["canopy_lb_2020_5ms" ,"canopy_lb_2020_10ms","canopy_lb_2020_15ms","canopy_lb_2020_25ms"]);
imgsFolder = string(["canopy_lb_2020_5ms_000000","canopy_lb_2020_10ms_000000","canopy_lb_2020_15ms_000000","canopy_lb_2020_25ms_000000"]);
dat = ["canopy_lb_2020_5ms_000000.dat","canopy_lb_2020_10ms_000000.dat","canopy_lb_2020_15ms_000000.dat","canopy_lb_2020_25ms_000000.dat"];
...
%% PROGRAM SWITCHES
ProgMode = 1; %1:5ms , 2:10ms , 3:15ms, 4:25ms
AnalyzeStandard = true;
IgnoreImg = false;
SegmentMode = true;
ReflectanceWrite = true;
ImgWriteMode = false;
AccumulateStandard = true;
StoreAvgMode = false;
%% DATA DEPENDENCIES
refSheet = readtable(fullfile(getenv('USERPROFILE'),'Documents','repos','hyperspectral','tripodAnalysis')+"\StSupery_Work_file",'ReadRowNames',true); % From local


if ProgMode == 1
    refAvg = readtable(fullfile(getenv('USERPROFILE'),'Documents','repos','hyperspectral','tripodAnalysis')+"\RadianceSheets\5msRadianceAVG",'ReadRowNames',true); %5ms
end
if ProgMode == 2
    refAvg = readtable(fullfile(getenv('USERPROFILE'),'Documents','repos','hyperspectral','tripodAnalysis')+"\RadianceSheets\10msRadianceAVG",'ReadRowNames',true); %10ms
end
if ProgMode == 3
    refAvg = readtable(fullfile(getenv('USERPROFILE'),'Documents','repos','hyperspectral','tripodAnalysis')+"\RadianceSheets\15msRadianceAVG",'ReadRowNames',true); %5ms
end
if ProgMode == 4
    refAvg = readtable(fullfile(getenv('USERPROFILE'),'Documents','repos','hyperspectral','tripodAnalysis')+"\RadianceSheets\25msRadianceAVG",'ReadRowNames',true); %5ms
end

%% LOOP, ACCUMULATE STANDARDS
if AccumulateStandard == true % This is used after calibration radiance avgs have been collected
    for i = 1:length(refAvg.Row)
        try
            id = string(refAvg.Row(i))
        catch
            id = "CorruptName" + string(refAvg.Row(i));
        end
...
if SegmentMode == true
    ...
    for i = 1:length(leafId) 
        ...
        % Load image
        try
            Cube = multibandread('D:\St_Supery_IOP1_Sep2020_contd\'+datPath,[1024,1024,240],'uint16',0','bsq','ieee-be'); % 5ms initially, to save memory
        catch
            error = "skipping, hypercube error on " + id
            continue;
        end
        % TO DO: HANDLE AN IMAGE WHICH IS LOADED INSTEAD AS AN OBJECT_MASK
        
        hCube = hypercube(Cube,wv);
        if isfile(fullfile(getenv('USERPROFILE'),'Documents','repos','hyperspectral','tripodAnalysis','ManualMasks',id+"mask.mat"))
            S = load(fullfile(getenv('USERPROFILE'),'Documents','repos','hyperspectral','tripodAnalysis','ManualMasks',id+"mask.mat"));
            try 
                mask = S.BW;
            catch
                try
        ...
        if StoreAvgMode == true
            ...
            %store in dir:
            %if ProgMode >= 1 %CHANGE this case later, (temporary for testing)
             %   writematrix([id string(avgxband)],fullfile(getenv('USERPROFILE'),'Documents','repos','hyperspectral','tripodAnalysis','RadianceSheets')+"\25msRadianceAVG.csv",'WriteMode','append');
            %end
        end
         %image write mode
       ...
       %Store average
            if ProgMode == 1
                writematrix([id string(avgxband)],fullfile(getenv('USERPROFILE'),'Documents','repos','hyperspectral','tripodAnalysis','ReflectanceSheets')+"\SeptTripod5msReflectanceAvg.csv",'WriteMode','append');
            end
            if ProgMode == 2
                writematrix([id string(avgxband)],fullfile(getenv('USERPROFILE'),'Documents','repos','hyperspectral','tripodAnalysis','ReflectanceSheets')+"\SeptTripod10msReflectanceAvg.csv",'WriteMode','append');
            end
            if ProgMode == 3
                writematrix([id string(avgxband)],fullfile(getenv('USERPROFILE'),'Documents','repos','hyperspectral','tripodAnalysis','ReflectanceSheets')+"\SeptTripod15msReflectanceAvg.csv",'WriteMode','append');
            end
            if ProgMode == 4
                writematrix([id string(avgxband)],fullfile(getenv('USERPROFILE'),'Documents','repos','hyperspectral','tripodAnalysis','ReflectanceSheets')+"\SeptTripod25msReflectanceAvg.csv",'WriteMode','append');
            end
        end
    end
