...
reflCube = zeros(603,681,80);%data cube to consist of reflectance values
avgxband = zeros(1,80);
idNames = char(leafId);
% averages belonging to the calibration standard at .99 reflectance
sRad = readtable('standardRadAvg.csv','ReadRowNames',true); %indexing: sRad{'row',[1:78]}
...
for i = 1:length(leafId)
    ...
    for j = 1:length(wv)
        ...
        reflCube(:,:,j) = Cube.DataCube(:,:,j); 
        reflCube(:,:,j) = reflCube(:,:,j).*leaf;
        reflCube(:,:,j) = reflCube(:,:,j).*dataGain(j);
        reflCube(:,:,j) = reflCube(:,:,j).*(.99/radOffset);
        %img = Cube.DataCube(:,:,j);
        %img = img.*leaf;
        %img = img.*dataGain(j);
